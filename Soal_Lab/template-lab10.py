...
class WindowBeliBarang(tk.Toplevel):
    ...
    def create_widgets(self):
        my_label = tk.Label(self, text="Form Beli Barang")
        my_label.grid(column=1, row=0)

        label_nama_barang = tk.Label(self, text="Nama Barang")
        label_nama_barang.grid(row=1, column=0)

        self.ent_nama_barang = tk.Entry(self)
        self.ent_nama_barang.grid(row=1, column=1)

        label_jumlah = tk.Label(self, text="Jumlah")
        label_jumlah.grid(row=2, column=0)
        ...
        self.btn_beli.grid(row=3, column=1)
    ...
    def beli_barang(self):
        ...
        if nama_barang == "":
            is_retry = tkmsg.askretrycancel(
                title="BarangNotFound", message="Nama barang tidak boleh kosong.")
            if not is_retry:
                self.destroy()

        elif nama_barang not in self.product_dict:
            is_retry = tkmsg.askretrycancel(
                title="BarangNotFound", message=f"Barang dengan nama {nama_barang} tidak ditemukan dalam BakungLapak.")
            if not is_retry:
                self.destroy()

        elif self.product_dict[nama_barang].get_stok() - jumlah < 0:
            tkmsg.showwarning(
                title="StokEmpty", message=f"Maaf, stok produk telah habis.")
        else:
...
class WindowCheckOut(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        if len(self.daftar_dibeli) <= 0:
            ...
            for product in sorted(self.daftar_dibeli.keys(), key=lambda product: product.get_nama()):
                
                tk.Label(self,
                        text=f"{product.get_nama()}").grid(row=i, column=0)
                tk.Label(self,
                        text=f"{product.get_harga()}").grid(row=i, column=1)
                tk.Label(self,
                        text=f"{self.daftar_dibeli[product]}").grid(row=i, column=2)
                i += 1
...
if __name__ == "__main__":
    ...
    product_dict = {"Kebahagiaan": Product("Kebahagiaan", 999999, 1),
                    ...
                    "Minuman": Product("Minuman",5000,4)}
