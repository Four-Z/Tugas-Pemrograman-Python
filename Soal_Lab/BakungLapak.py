...
class WindowBeliBarang(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        tk.Label(self, text='Form Beli Barang').grid(row=0, column=2)
        tk.Label(self, text='Nama Barang').grid(row=1, column=1)
        tk.Entry(self, textvariable=self.ent_nama_barang).grid(row=1, column=2)
        tk.Label(self, text='Jumlah').grid(row=2, column=1)
        tk.Entry(self, textvariable=self.ent_jumlah).grid(row=2, column=2)
        tk.Button(self, text="BELI", command=self.beli_barang).grid(
            row=3, column=2)
    ...
    def beli_barang(self):
        ...
        if nama_barang == "":
            ...
            check = tkmsg.askretrycancel(
                "BarangNotFound", "Nama barang tidak boleh kosong.")
            if not check:
                self.destroy()
        elif nama_barang not in self.product_dict:
            # TODO : jika barang tidak ditemukan
            check = tkmsg.askretrycancel(
                "BarangNotFound", f"Barang dengan nama {nama_barang} tidak ditemukan dalam BakungLapak")
            if not check:
                self.destroy()
        elif self.product_dict[nama_barang].get_stok() - jumlah < 0 or jumlah <= 0:
            # TODO : jika stok habis
            tkmsg.showinfo("StockEmpty", "Maaf, stok produk telah habis")
        else:
            ...
            self.ent_jumlah.set("")
            tkmsg.showinfo(
                "Berhasil!", f"Berhasil membeli {nama_barang} sejumlah {jumlah}")
...
class WindowCheckOut(tk.Toplevel):
    ...
    def create_widgets(self):
        # TODO: lengkapi method ini
        tk.Label(self, text='Keranjangku').grid(row=0, column=2)
        tk.Label(self, text='Nama Produk').grid(row=1, column=1)
        tk.Label(self, text='Harga Barang').grid(row=1, column=2)
        tk.Label(self, text='Jumlah').grid(row=1, column=3)

        i = 2
        check = True
        for barang in sorted(self.daftar_dibeli.keys(), key=lambda x: x.get_nama()):
            check = False
            tk.Label(self,
                     text=f"{barang.get_nama()}").grid(row=i, column=1)
            tk.Label(self,
                     text=f"{barang.get_harga()}").grid(row=i, column=2)
            tk.Label(self,
                     text=f"{self.daftar_dibeli[barang]}").grid(row=i, column=3)
            i += 1

        if check:
            tk.Label(self,
                     text="belum ada barang yang dibeli :(").grid(row=2, column=2)

        self.btn_exit = tk.Button(self, text="EXIT",
                                  command=self.destroy).grid(row=i+1, column=2)
